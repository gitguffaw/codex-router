...
function makeVersionFixture() {
  ...
  writeJson(path.join(root, "package.json"), {
    name: "@compound/codex-router-plugin-cc",
    version: "1.0.2"
  });
  ...
  writeJson(path.join(root, "plugins", "codex-router", ".claude-plugin", "plugin.json"), {
    name: "codex-router",
    version: "1.0.2"
  });
...
test("bump-version updates every release manifest", () => {
  ...
  assert.equal(readJson(path.join(root, ".claude-plugin", "marketplace.json")).version, "1.2.3");
...
test("bump-version check mode reports stale metadata", () => {
  ...
  assert.match(result.stderr, /\.claude-plugin\/marketplace\.json version/);
