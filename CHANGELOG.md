...
- [2.5.1](./plugins/codex-router/CHANGELOG.md#251): startup output is `Codex is ready.` or a failure that blocks Codex. MCP connection failures, launch details, and other non-blocking startup errors stay out of the caller response.
- [2.5.0](./plugins/codex-router/CHANGELOG.md#250): one launch table for analyze/exec/review/task, `command` as the persisted job kind, `--wait` only on status, and `/review` with focus text errors instead of promoting.
